...
- `num_classes`：COCO是91（包括背景？不，这里的`num_classes+1`指的是**真实类别数 + 1个“无物体”类**。代码注释中明确写着：“including no-object category”）

每个查询经过解码器后得到一个256维的特征向量`hs`（形状`[num_queries, batch, 256]`），分类头将其映射到 `(num_classes+1)` 维的logits。最后的 `no-object` 类别用于表示“这个查询没有对应任何物体”——这替代了NMS的功能。
...
- **结构**：3层全连接（256→256→256→4），每层之间用ReLU激活。
...
全景分割头并不是另起炉灶，而是在**检测头的基础上增加了两个新模块**（见图1，图中展示了从检测头、注意力图到上采样卷积的流程）：
...
**它做了什么？**  
输入：
- `q`：解码器最后一层输出的查询特征（`[num_queries, batch, hidden_dim]`）
- `k`：编码器输出的记忆特征（`[batch, hidden_dim, H, W]`）
...
输出形状 `[batch, num_queries, 8, H/32, W/32]`（8个注意力头）。每个头捕捉不同的空间关系（比如一个头关注物体中心，另一个头关注边界）。
...
- **`x`**：骨干网络（ResNet）**最后一层**输出的特征图，形状为 `[batch, 2048, H/32, W/32]`，经 `self.detr.input_proj`（一个1×1卷积）将通道数压缩到 `hidden_dim=256`，得到 `[batch, 256, H/32, W/32]`。  
  注意：这里是骨干网络的**最后一层**（分辨率最低，语义最强），而非编码器的输出。编码器输出虽然也包含全局信息，但为了保留最原始的高层语义，DETR选择直接使用投影后的骨干特征作为上采样的起点。

- **`bbox_mask`**：上面计算得到的注意力权重，形状 `[batch, num_queries, 8, H/32, W/32]`。每个查询对应8个注意力头，每个头是一张空间响应图。

- **`fpns`**：ResNet骨干网络**前三层**的输出（`[1024, 512, 256]` 通道，分辨率依次为 `H/16, H/8, H/4`）。为什么不用最后一层？因为最后一层已经用作Transformer的输入（并被投影到256维），而前三层**分辨率更高**，包含丰富的**边缘和纹理细节**，这对生成精确掩码至关重要。这就像一个画家先画出大轮廓（最后一层语义），再用细笔触补充细节（前三层高分辨率特征）。
...
- `_expand(x, n)`：`x.unsqueeze(1).repeat(1, n, 1, 1, 1).flatten(0,1)`，结果形状为 `[batch * num_queries, 256, H/32, W/32]`。
- `bbox_mask.flatten(0,1)`：将 `[batch, num_queries, 8, H/32, W/32]` 展平为 `[batch * num_queries, 8, H/32, W/32]`。

然后在通道维拼接，得到 `[batch * num_queries, 256 + 8, H/32, W/32]`。这里的 **264 = 256（骨干特征通道数）+ 8（注意力头数）**。**8个注意力头的权重图被当作8个独立的特征通道**，与骨干特征拼接后输入卷积层。这样设计的好处是：让卷积层自己学习如何根据不同头的空间响应来调整掩码生成——有些头可能关注物体中心，有些头关注边界，网络可以自适应地融合这些信息。

#### （3）上采样的网络结构：标准FPN的逐级相加融合

`MaskHeadSmallConv` 设计了5个卷积块（`lay1`~`lay5`），每个块包含 `Conv2d + GroupNorm(8) + ReLU`。同时，通过3个适配器（`adapter1`~`adapter3`，均为1×1卷积）将FPN三层特征映射到对应的中间通道数，并**逐级相加**实现特征融合和上采样。
...
- 输入：`[batch*num_queries, 264, H/32, W/32]`
- `lay1` + `lay2`：保持分辨率，通道 264 → 128 → 64
...
- `lay3`：处理 `H/16` 特征，通道 64 → 32
- 与 `fpns[1]`（分辨率 `H/8`，经 `adapter2` 映射到32通道）相加，上采样到 `H/8`
- `lay4`：通道 32 → 16
- 与 `fpns[2]`（分辨率 `H/4`，经 `adapter3` 映射到16通道）相加，上采样到 `H/4`
- `lay5` + `out_lay`：保持 `H/4`，通道 16 → 16 → **1**

最终输出形状：`[batch * num_queries, 1, H/4, W/4]`，再通过 `view(bs, num_queries, H/4, W/4)` 得到每个查询的**粗糙掩码**。后续后处理（`PostProcessSegm` 或 `PostProcessPanoptic`）会将其双线性插值到原图分辨率。
...
- **检测头**：`class_embed` 输出 251 维（COCO全景分割类别数 250 + 1 个“无物体”类），`bbox_embed` 是 3 层 MLP 输出 4 维坐标。完全符合设计。
- **注意力模块**：`MHAttentionMap` 的 Q/K 线性层输入输出均为 256 维，8 个注意力头（`nheads=8`，但打印中未显式出现，由传入参数决定）。
- **分割头上采样模块**：第一个卷积层输入通道为 264，印证了 **264 = 256（骨干特征）+ 8（注意力头）**。后续通道逐级减半（128→64→32→16），最终输出单通道掩码。
...
- **`x`**：骨干最后一层（`layer4`）经 1×1 卷积投影后，形状 `(1,256,21,42)`。原图尺寸假设为 `(672, 1344)`，下采样 32 倍后得到 `21×42`。
- **`bbox_mask`**：注意力模块输出的 8 头权重图，形状 `(1,100,8,21,42)` —— 100 个查询，每个查询有 8 张空间响应图。
- **`fpns`**：骨干前三层输出，分辨率依次为原图的 1/16（41×84）、1/8（82×167）、1/4（164×333）。这些高分辨率特征将为上采样提供细节。
...
- 经过 `lay1` 和 `lay2` 两层卷积（通道 264→128），分辨率仍为 21×42。此时特征图已融合了查询的注意力信息。
...
- `adapter1` 将 `fpns[0]`（1024 通道，41×84）映射到 128 通道，得到 `cur_fpn` 形状 `(1,128,41,84)`。由于 `x` 的 batch 为 100，需要将 `cur_fpn` 复制 100 份（`_expand`）。
- 当前 `x` 形状为 `(100,128,21,42)`，先上采样到 41×84，再与 `cur_fpn` 相加（**FPN 首次融合**），然后经 `lay3` 卷积降维到 64 通道。分辨率提升到原图的 1/16。
...
- `adapter2` 将 `fpns[1]`（512 通道，82×167）映射到 64 通道，上采样当前 `x`（64 通道，41×84）到 82×167，相加后经 `lay4` 降维到 32 通道。分辨率提升到 1/8。
...
- `adapter3` 将 `fpns[2]`（256 通道，164×333）映射到 32 通道。当前 `x`（32 通道，82×167）上采样到 164×333 后相加，经 `lay5` 降维到 16 通道。
...
| 组件 | 设计要点 |
|------|-----------|
| **分类头** | 一个线性层，将解码器输出映射到 `(num_classes + 1)` 维（+1 表示无物体类）。配合匈牙利匹配，自动抑制冗余框。 |
| **框回归头** | 3层MLP（256→256→256→4），输出归一化的 `(cx, cy, w, h)`，经 sigmoid 压缩到 `[0,1]`。 |
...
| 组件 | 设计要点 |
|------|-----------|
| **注意力模块 (`MHAttentionMap`)** | 只计算注意力权重，不乘V。输出 `[batch, num_queries, 8, H/32, W/32]` 的8头空间响应图，用于指导掩码生成。 |
